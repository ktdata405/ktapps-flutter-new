...
class _CashewScreenState extends State<CashewScreen> {
  ...
  // ── import preview ────────────────────────────────────────────────────────
  Map<String, List<_ImportEntry>>? _importedGrouped;
  ...
  void _addDefaultRows() {
    for (final cat in ['Home', 'My Family', 'My Personal']) {
      _rows.add(_ExpenseRow(category: cat));
  ...
  void _removeRow(int index) {
    if (_rows.length <= 1) {
      setState(() {
        ...
        _rows[0] = _ExpenseRow(category: 'Select Category');
      });
      return;
    }
    setState(() {
  ...
  Future<void> _fetchDataForDate(DateTime date) async {
    ...
    try {
      ...
      final url =
          '$_cashewSheetUrl?sheetName=${Uri.encodeComponent(sheetName)}&t=${DateTime.now().millisecondsSinceEpoch}';
      final response = await http.get(Uri.parse(url));
      final data = jsonDecode(response.body) as Map<String, dynamic>;
      final allRows = (data['data'] as List? ?? []);
      final forDate =
          allRows.where((e) => (e['date'] ?? '') == formattedDate).toList();
      ...
      await _fetchDatesForCalendar(date);
    } catch (e) {
  ...
  Future<void> _fetchDatesForCalendar(DateTime date) async {
    try {
      final sheetName = _sheetNameFromDate(date);
      final url =
          '$_cashewSheetUrl?sheetName=${Uri.encodeComponent(sheetName)}&datesOnly=true';
      final response = await http.get(Uri.parse(url));
      final data = jsonDecode(response.body) as Map<String, dynamic>;
  ...
  void _calcResult() {
    try {
      ...
      setState(() {
        _calcHistory = '$expr =';
        _calcDisplay =
            r.toStringAsFixed(r.truncateToDouble() == r ? 0 : 6);
  ...
  Future<void> _triggerImport() async {
    ...
    try {
      ...
      for (final sheetName in excel.tables.keys) {
        ...
        final remarksIdx =
            colIdx(['remarks', 'remark', 'narration', 'description', 'details']);
  ...
  Widget _buildBgGlows() {
    return Stack(children: [
      Positioned(
          top: -60,
          left: -60,
          child: _glowCircle(const Color(0xFF4F46E5), 0.2)),
      Positioned(
          bottom: -60,
          right: -60,
          child: _glowCircle(const Color(0xFFDB2777), 0.2)),
    ]);
  }
  ...
  Widget _buildCalendarAccordion() => Container(
        ...
        child: Column(children: [
          ...
          if (_calendarOpen) ...[
            ...
            Padding(padding: const EdgeInsets.all(8), child: _buildCalGrid()),
            Padding(
  ...
  Widget _buildExpenseRow(int i) => Padding(
              ...
              Container(
                ...
                child:
                  ...
                  const Padding(
                      ...
                      child: Icon(Icons.notes, color: _textGray500, size: 16)),
                  Expanded(
                      child: TextField(
                    controller: _descControllers[i],
                    maxLines: 3,
                    style: const TextStyle(
                        color: _textGray400, fontSize: 13, height: 1.5),
                    decoration: const InputDecoration(
                    ...
                    onChanged: (_) => _parseAndCalculate(i),
                  )),
                ]),
              ),
            ]),
          ...
          Positioned(
            ...
            child: GestureDetector(
              ...
              child: Container(
                  ...
                  height: 28,
                  decoration:
                      const BoxDecoration(color: _rose, shape: BoxShape.circle),
  ...
  Widget _buildBottomBar() {
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Row(children: [
          Expanded(
              child: _actionBtn(Icons.add, 'Add',
                  Colors.white.withValues(alpha: 0.05), _textGray400,
                  onTap: () => _addRow())),
          const SizedBox(width: 8),
          GestureDetector(
            onTap: _clearAll,
            child: Container(
              ...
              decoration: BoxDecoration(
                  ...
                  border: Border.all(color: _borderWhite5)),
              child:
                  const Icon(Icons.delete_outline, color: _textGray400, size: 20),
            ),
          ),
          const SizedBox(width: 8),
          Expanded(
              child: _actionBtn(Icons.edit_note, 'Draft',
                  Colors.grey.withValues(alpha: 0.3), Colors.white,
  ...
  Widget _actionBtn(IconData icon, String label, Color? bg, Color tc,
          {bool gradient = false, required VoidCallback onTap}) =>
      GestureDetector(
        onTap: onTap,
        child: Container(
          height: 44,
          decoration: BoxDecoration(
            ...
            gradient: gradient
                ...
                : null,
            borderRadius: BorderRadius.circular(16),
            border: gradient ? null : Border.all(color: _borderWhite5),
            boxShadow: gradient
                ? [
                    BoxShadow(
  ...
  Widget _calcButtons() {
    ...
    final opBg = _primary.withValues(alpha: 0.15);
    final clearBg = _rose.withValues(alpha: 0.15);
  ...
  Widget _buildImportModal() {
    ...
    final current =
        List<_ImportEntry>.from(grouped[_importActiveDate] ?? [])
                      ...
                      children: [
                        Text(_importActiveDate ?? '',
                            style: const TextStyle(
                                ...
                                fontWeight: FontWeight.w700,
                                fontSize: 13)),
                        Text('${current.length} | ₹${selTotal.toStringAsFixed(2)}',
                            style: const TextStyle(
                                color: _emerald,
                                fontSize: 11,
                                fontFamily: 'monospace')),
